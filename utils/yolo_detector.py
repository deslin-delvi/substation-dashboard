...
class YOLOProcessor:
    ...
    def update_gate_state(self, new_gate_state):
        ...
        if self.prev_gate_state != new_gate_state:
            ...
            if new_gate_state == "CLOSED":
                ...
                if ppe_status == "NOT_OK" and self.prev_gate_state == "OPEN":
                    self.capture_gate_violation(
    ...
    def _process_results(self, results):
        """
        🔧 UPDATED: Only updates PPE status, does NOT capture photos
        Photos are only captured via capture_gate_violation()
        """
        ...
        if new_status == "NOT_OK" and self.prev_status != "NOT_OK":
            ...
            if elapsed >= self.startup_grace_period:
                ...
                print(f"⚠️ PPE Status: NOT_OK (missing: {missing}) - No photo captured")
